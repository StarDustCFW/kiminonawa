...
void auto_launch_update()
{
	...
	else if (sd_mount())
	{
		if (!f_stat("stardust/update.bin", NULL))
			launch_payload("stardust/update.bin", true);
	}
}
...
void launch_tools(u8 type)
{
	...
	if (sd_mount())
	{
		...
		if (!type)
			memcpy(dir, "stardust/payloads", 18);
		else
			memcpy(dir, "stardust/modules", 17);
...
void launch_firmware()
{
	...
	if (sd_mount())
	{
		if (ini_parse(&ini_sections, "stardust/kronos_ipl.ini", false))
		{
			// Build configuration menu.
			...
			EPRINTF("Could not open 'stardust/kronos_ipl.ini'.\nMake sure it exists!");
	}
...
void auto_launch_firmware()
{
	...
	if (sd_mount())
	{
		if (f_stat("stardust/kronos_ipl.ini", NULL))
			create_config_entry();
	...
	if (!(b_cfg.boot_cfg & BOOT_CFG_FROM_LAUNCH) && h_cfg.bootwait)
	{
		...
		if (!bitmap) // Custom entry bootlogo not found, trying default custom one.
			bitmap = (u8 *)sd_file_read("stardust/bootlogo.bmp", NULL);
	...
	// Wait before booting. If VOL- is pressed go into stardust menu.
	if (!h_cfg.sept_run)
	{
...
void about()
{
	static const char credits[] =
		...
		"  Backup Mode by Kronos2308\n"
		" ___________________________________________\n\n"
		"  Original source:\n"
		"  %khttps://github.com/CTCaer/hekate%k\n"
		" ___________________________________________\n\n";
...
ment_t ment_restore[] = {
	MDEF_BACK(),
	MDEF_CHGLINE(),
	MDEF_CAPTION("------ Completo --------", 0xFF0AB9E6),
	MDEF_HANDLER("Restaurar los BOOT0/1", restore_emmc_boot),
	MDEF_HANDLER("Restaurar rawnand.bin (o piezas)", restore_emmc_rawnand),
	MDEF_CHGLINE(),
	MDEF_CAPTION("-- Particiones --", 0xFF0AB9E6),
	MDEF_HANDLER(" Restaurar Particiones individuales", restore_emmc_gpp_parts),
	MDEF_END()
};

menu_t menu_restore = {
	ment_restore,
	"Opciones de Restauracion", 0, 0

};

ment_t ment_restoreen[] = {
	MDEF_BACK(),
	MDEF_CHGLINE(),
...
ment_t ment_topen[] = {
	MDEF_CAPTION("------ Backup Mode --------", 0xFF0AB9E6),
	MDEF_HANDLER("Backup - nand and boot0/1", backupfull),
	MDEF_CAPTION("Take time (Look for popcorn)", 0xFF0AB9E6),
	MDEF_CAPTION("--------------------", 0xFF444444),
	MDEF_HANDLER("Backup lite (Advanced User)", backuplite),
	MDEF_CAPTION("--------------------", 0xFF444444),
	MDEF_MENU("Restore Menu", &menu_restoreen),
	MDEF_CAPTION("--------------------", 0xFF444444),
	MDEF_HANDLER("Payloads...",launch_tools_payload),
//	MDEF_HANDLER("See burned efuses", print_fuseinfo),
	MDEF_HANDLER("AutoRCM", menu_autorcm),
	MDEF_CAPTION("--------------------", 0xFF444444),
	MDEF_HANDLER("Fix the Archive bit", fix_sd_all_attr),
	MDEF_CAPTION("--------------------", 0xFF444444),
	MDEF_HANDLER("Power off", power_off),
	MDEF_HANDLER("Back to boot menu", mainmenu),
	MDEF_CAPTION("--------------------", 0xFF444444),
...
menu_t menu_topen = {
	ment_topen,
	"hekate v4.10.1 backup mode by Kronos2308", 0, 0
};

ment_t ment_top[] = {
	MDEF_CAPTION("------ Modo Backup --------", 0xFF0AB9E6),
	MDEF_HANDLER("Backup - nand Y boot0/1", backupfull),
	MDEF_CAPTION("Toma tiempo (Busca palomitas)", 0xFF0AB9E6),
	MDEF_CAPTION("--------------------", 0xFF444444),
	MDEF_HANDLER("Backup Rapido System", backuplite),
	MDEF_CAPTION("--------------------", 0xFF444444),
	MDEF_MENU("Restaurar nand", &menu_restore),
	MDEF_CAPTION("--------------------", 0xFF444444),
	MDEF_HANDLER("Payloads...",launch_tools_payload),
//	MDEF_HANDLER("Ver efuses Quemados", print_fuseinfo),
	MDEF_HANDLER("AutoRCM", menu_autorcm),
	MDEF_CAPTION("--------------------", 0xFF444444),
	MDEF_HANDLER("Arreglar el bit de Archive", fix_sd_all_attr),
	MDEF_CAPTION("--------------------", 0xFF444444),
	MDEF_HANDLER("Apagar", power_off),
	MDEF_HANDLER("Volver al boot menu", mainmenu),
	MDEF_CAPTION("--------------------", 0xFF444444),
	MDEF_HANDLER("Acerca de..", about),
	MDEF_CAPTION("--------------------", 0xFF444444),
	MDEF_MENU("Change Text to English", &menu_topen),
	MDEF_END()
};
...
void ipl_main()
{
	...
	// Set stardust's default configuration.
	set_default_configuration();

	// Save sdram lp0 config.
	if (ianos_loader(true, "stardust/sys/libsys_lp0.bso", DRAM_LIB, (void *)sdram_get_params_patched()))
