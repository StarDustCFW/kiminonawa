...
void dump_packages12()
{
	...
	if (kb <= KB_FIRMWARE_VERSION_620)
	{
		...
		// Display info.
		gfx_printf("%kNX stardust size:  %k0x%05X\n\n", 0xFFC7EA46, 0xFFCCCCCC, hdr->ldr_size);
		...
		if (sd_save_to_file(loader, hdr->ldr_size, path))
			goto out_free;
		gfx_puts("NX stardust dumped to nxloader.bin\n");
...
void minerva()
{
	...
	ianos_loader(false, "stardust/sys/libsys_minerva.bso", DRAM_LIB, (void *)&mtc_cfg);
